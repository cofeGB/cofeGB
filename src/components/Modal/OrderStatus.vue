    ...
    <template #content>
      <div class="overlay" @click.self="close()">
        <div class="wrapper">
          ...
          <div class="mt-4 status" v-for="{ order, index } in USER_ORDERS()" :key="index">
...
export default {
  ...
  methods: {
    ...mapActions(['CLOSE_MODAL', 'CLEAN_USER_ORDERS']),
    ...mapGetters(['ORDER_STATUS', 'USER_ORDERS']),
    findOrder() {
      this.$store.dispatch('GET_ORDERS_BY_PHONE', this.userPhone);
      let orders = this.USER_ORDERS();
      if (orders.length === 0) {
        this.notFound = true;
      }
    },
